...
import CutIcon from "../../assets/icons/CutIcon.tsx"
...
import { Cut } from "../../stores/OperationStores/CutStore.tsx"
...
export const CutDialog = observer(() => {
    ...
    return(
        <React.Fragment>
        <Button sx={{height: 60,width: 60, backgroundColor:"#2D2D2D"} } onClick={handleClickCut}><CutIcon></CutIcon></Button>
        <Modal open={open} onClose={handleClickCut}>
                    <Box sx={style}>
                        <Grid2 container columnSpacing={{ xs: 1, sm: 2, md: 3 }}  rowGap={1}>
                            ...
                            var cut = new Cut(parseFloat(text_one), parseFloat(text_two), alignment);
                            selectedPart?.opStore.corners.addOperation(cut, alignment)
                            handleClickCut()
